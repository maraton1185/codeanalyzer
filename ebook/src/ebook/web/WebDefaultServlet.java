 ...
 * 
 */
public class WebDefaultServlet extends HttpServlet implements ResourceFactory {
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			// Handle resource
			boolean isRoot = pathInContext.equalsIgnoreCase("/");
			if ((resource == null || !resource.exists()) && !isRoot) {
				if (included)
					throw new FileNotFoundException("!" + pathInContext);
				response.sendError(HttpServletResponse.SC_NOT_FOUND);
			} else if (!resource.isDirectory() && !isRoot) {
	...
	private String getWelcomeFile(String pathInContext)
		...
		for (int i = 0; i < _welcomes.length; i++) {
			...
			if ((_welcomeServlets || _welcomeExactServlets)
				...
				MappedEntry<?> entry = _servletHandler
		...
		try {
			if (!HttpMethod.HEAD.is(request.getMethod())) {
				if (_etags) {
					...
					String if_non_match_etag = request
		...
		if (reqRanges == null || !reqRanges.hasMoreElements()
			...
			if (ranges.size() == 1) {
				...
				if (!response.containsHeader(HttpHeader.DATE.asString()))
					response.addDateHeader(HttpHeader.DATE.asString(),
			...
			String mimetype = (content == null ? null : content
					.getContentType());
			if (mimetype == null)
	...
	protected void writeHeaders(HttpServletResponse response,
			HttpContent content, long count) {
