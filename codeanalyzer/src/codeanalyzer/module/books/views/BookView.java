...
public class BookView {
	...
	public void EVENT_UPDATE_BOOK_INFO(
			...
			@Optional CurrentBookInfo book, IBookManager bm,
	...
	@Persist
	public void save(@Optional CurrentBookInfo book, IBookManager bm,
			Shell shell) {
		if (book == null)
			return;
	...
	public void postConstruct(Composite parent) {
		...
		model = BeanProperties.value(CurrentBookInfo.class, "opened")
		...
		model = BeanProperties.value(CurrentBookInfo.class, "viewMode")
				.observeDetail(bookValue);
		ctx.bindValue(target, model);

		target = WidgetProperties.selection().observe(radio2);
		model = BeanProperties.value(CurrentBookInfo.class, "editMode")
				.observeDetail(bookValue);
		ctx.bindValue(target, model);
